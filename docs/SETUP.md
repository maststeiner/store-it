...
- [x] **License policy** (2026-07-16): project licensed under **MIT**. Dependencies: permissive licenses only (MIT, Apache-2.0, BSD, ISC); copyleft/special clauses (GPL, AGPL, LGPL, SSPL) are blocked — enforced twice: Trivy license scan (repo-wide) + dependency-review-action (PR diff). Scope (2026-08-03, #64): the copyleft deny-list governs what we **ship** (npm/nuget runtime dependencies); CI-only GitHub Actions are build tooling that is never distributed, so copyleft does not apply to them and they are exempted from the deny-list via `allow-dependencies-licenses` (e.g. the LGPL-licensed `sonarqube-scan-action`).
