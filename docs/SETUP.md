...
- [x] `renovate.json` on the **default branch** (`develop` since 2026-08-18): nightly **branch-creation window** (`schedule:daily` → cron `* 0-3 * * *`, timezone `Europe/Zurich`; was a weekly Monday window until 2026-08-06). `schedule` gates when Renovate may create/update branches and PRs — not how often the hosted app runs, which is its own cadence. PRs target `develop` only, prConcurrentLimit 3 — normally max. 3 Renovate PRs open at once, aligned with the WIP limit.
