...
- [ ] **[platform] Activate S138 in SonarCloud's C# quality profile** (decision 2026-08-07: enforce it *on both sides*). It is enforced locally as of #74, but SonarCloud never reported it — the built-in *Sonar way* profile does not include it, so Gate 3 is currently the more permissive of the two. To close the gap: SonarCloud → Organization `maststeiner` → **Quality Profiles** → C# → *Sonar way* → **Extend** (a built-in profile cannot be edited) → name it e.g. `store-it C#` → activate rule **`csharpsquid:S138`** (*"Methods should not have too many lines"*, default max 80) → set the new profile as the one used by `maststeiner_store-it-backend`. Optional, for the frontend: the same rule exists as `typescript:S138`.
