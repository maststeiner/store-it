...
| 6 | Automated review of the implementation, four rounds | Eleven findings, all fixed and answered on the PR: doc drift after the `compose.yaml` fix, a start script that reported success after a failed readiness loop, `cp` clobbering `.env`, `curl` assumed present, hard-coded ports in three places (README setup, the redirect-inspection command, the `.env.example` callbacks), a teardown that deletes the database without saying so, an engine pinned for start but not for teardown, and `localhost` documented where only IPv4 is published. One of them was a real behaviour bug: `.env.example` shipped `POSTGRES_PASSWORD=storeit`, so the spec's "absent must fail loudly" (config table, EC) could never fire — the template now ships it empty and `compose.stack.yaml`'s `:?` guard reports it by name before anything starts. |
